import { between, capitalize } from 'quasar-framework/src/utils/format.js'
import { position, stopAndPrevent } from 'quasar-framework/src/utils/event.js'
import { css } from 'quasar-framework/src/utils/dom.js'
import { isSameDate, adjustDate, pMonths } from './date.js'
import DateMixin from './datetime-mixin.js'
import ParentFieldMixin from 'quasar-framework/src/mixins/parent-field.js'
import TouchPan from 'quasar-framework/src/directives/touch-pan.js'
...
export default {
  ...
  methods: {
    ...
    setYear (value) {
      if (this.editable) {
        this.model = new PersianDate(this.model.year(this.__parseTypeValue('year', value)))
      }
    },
    setMonth (value) {
      if (this.editable) {
    ...
    setDay (value) {
      if (this.editable) {
        this.model = new PersianDate(this.model.date(this.__parseTypeValue('date', value)))
      }
    },
    ...
    setHour (value) {
      if (this.editable) {
        this.model = new PersianDate(this.model.hours(this.__parseTypeValue('hour', value)))
      }
    },
    setMinute (value) {
      if (this.editable) {
        this.model = new PersianDate(this.model.minutes(this.__parseTypeValue('minute', value)))
      }
    },
