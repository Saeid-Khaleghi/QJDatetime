import { height, width, offset } from 'quasar-framework/src/utils/dom.js'
import { position, stopAndPrevent, getEventKey } from 'quasar-framework/src/utils/event.js'
import QBtn from 'quasar-framework/src/components/btn/QBtn.js'
import { isSameDate, isValid, adjustDate, pMonths} from './date.js'
import DateMixin from './datetime-mixin.js'
import CanRenderMixin from 'quasar-framework/src/mixins/can-render.js'
import ParentFieldMixin from 'quasar-framework/src/mixins/parent-field.js'
import Ripple from 'quasar-framework/src/directives/ripple.js'
...
export default {
  ...
  computed: {
    ...
    fakeModel () {
      return new PersianDate([this.fakeYear, this.fakeMonth, 1])
    },
    fakeYear () {
    ...
    daysInMonth () {
      ...
      return new PersianDate([this.fakeYear, this.fakeMonth, 0]).daysInMonth()
    },

    monthString () {
      return `${pMonths[this.month - 1]}`
    },
    monthStamp () {
      return `${pMonths[this.fakeMonth - 1]} ${this.fakeYear}`
    },
    weekDayString () {
      return this.headerLabel || this.$q.i18n.date.days[(this.model.day() + 5 ) % 7]
    },

    fillerDays () {
      let days = ((this.fakeModel.day() - this.computedFirstDayOfWeek + 5) % 7)
      if (days < 0) {
        days += 7
      }
      return days
    ...
    beforeMinDays () {
      ...
      const
        pminYear = this.pmin.year(),
        pminMonth = this.pmin.month()
    ...
    afterMaxDays () {
      ...
      const
        pmaxYear = this.pmax.year(),
        pmaxMonth = this.pmax.month()
    ...
    maxDay () {
      return this.pmax !== null ? this.pmax.date() : this.daysInMonth
    },
    dateInterval () {
  ...
  methods: {
    ...
    setYear (value, skipView) {
      if (this.editable) {
        ...
        this.model.year(this.__parseTypeValue('year', value))
      }
    },
    ...
    setDay (value, skipView, year, month) {
      if (this.editable) {
        if (year && month) {
          ...
          fake.year(this.__parseTypeValue('year', year))
          fake.date(this.__parseTypeValue('date', value))
          this.model = fake
        }
        else {
          this.model.date(this.__parseTypeValue('date', value))
        }
    ...
    setHour (value) {
      ...
      this.model = new PersianDate(new PersianDate(this.model).hours(value))
    },
    setMinute (value) {
      if (!this.editable) {
        return
      }
      this.model = new PersianDate(new PersianDate(this.model).minutes(this.__parseTypeValue('minute', value)))
    },
